...
WriteTrellixBlockRules() {
  ...
  cat >"$TARGET_DIR/trellix-block.conf" <<'INNER_EOF'
...
UpdateAdsRule() {
  ...
  if [ -s "$TMP_FILE" ]; then
    ...
    LOG_OUT "规则下载成功: 共 $(wc -l <"$TARGET_DIR/awavenue-ads.conf") 条 → $TARGET_DIR/awavenue-ads.conf"
  else
    rm -f "$TMP_FILE"
    if [ -s "$BACKUP_FILE" ]; then
      cp "$BACKUP_FILE" "$TARGET_DIR/awavenue-ads.conf"
      LOG_OUT "下载失败，已从备份还原旧规则"
    else
...
[ $? -eq 0 ] && info "脚本已生成: $SCRIPT_PATH" || fail "脚本生成失败"

section "配置执行权限..."
chmod +x "$SCRIPT_PATH" && info "已赋予可执行权限" || fail "chmod 失败"
