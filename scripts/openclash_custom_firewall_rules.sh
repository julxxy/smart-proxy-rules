...
#
#   注意事项：
#     - 需要设备能访问 fastly.jsdelivr.net（国内一般可用）
#     - 下载失败时会保留上次规则，不会清空
...
UpdateAdsRule() {
  ...
  WaitForNetwork() {
    local RETRY=0
    until ping -c 1 -W 2 fastly.jsdelivr.net >/dev/null 2>&1; do
      RETRY=$((RETRY + 1))
  ...
  if curl -sf --max-time 30 \
    "https://fastly.jsdelivr.net/gh/TG-Twilight/AWAvenue-Ads-Rule@main/Filters/AWAvenue-Ads-Rule-Dnsmasq.conf" \
